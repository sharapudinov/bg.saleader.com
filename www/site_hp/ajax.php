			...
			if(!empty($_GET["multi"]) && !empty($_GET['id'])){
				...
				if(Add2BasketByProductID(intval($_GET["id"]), intval($_GET["q"]), false)){
					...
					$jStr = '
						...
						"SUM":"'.addslashes(CCurrencyLang::CurrencyFormat(round($basketQty["PRICE"]) * round($basketQty["QUANTITY"]), $basketQty["CURRENCY"], true)).'",
	...
	if(!$rsLastOffer->SelectedRowsCount()){
		...
		if($obReturnOffer = $rsLastOffer->GetNextElement()){
			...
			if(!empty($productFilelds["DETAIL_PICTURE"])){
				...
				$arImageResize = CFile::ResizeImageGet($arImageInfo, $arProductImage, BX_RESIZE_IMAGE_PROPORTIONAL, false);
